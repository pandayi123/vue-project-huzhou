<template>
  <div class="hardware-settings-container" v-loading="el_loading" element-loading-text="正在获取硬件地址...请稍后"
    ...
    <div class="button-container" style="margin-top: 0.9rem">
...
<script setup>
import { ref, reactive, nextTick, watch, toRaw, defineAsyncComponent, onMounted, onUnmounted } from 'vue'
...
const applySettings = async (type) => {
  ...
  switch (type) {
    case 'lock':
      loadingRef = isApplyingLock
      await apply_lock_settings(form_lock)
      break
    case 'switch':
      loadingRef = swith_is_applying
