...
// 通用的删除确认函数
const confirmDeleteAction = async (title, content, actionType) => {
  audioStore.play('/audio/警告音.mp3') // 播放警告音
  ...
  try {
    await ElMessageBox.confirm(content, title, {
      ...
      type: 'warning',
      customClass: 'sys-config-message-box warning-mode', // 复用您现有的红色弹窗样式
      center: true,
      draggable: true,
    })
    ...
    // 调用后端接口 (此处模拟调用，请替换为您的真实API)
    // 假设您的 electronAPI 支持 clearData 动作
    const response = await window.electronAPI.el_post({
      action: 'delete', // 或者 'clear_database'
      payload: {
        type: actionType, // 'face' or 'fingerprint'
      },
    })
    ...
    if (response.success) {
      audioStore.play('/audio/操作成功.mp3')
    } else {
...
const handleClearFaceDb = () => {
  confirmDeleteAction('清空人脸库', '此操作将永久删除所有已录入的人脸数据，且不可恢复！是否继续？', 'face_db')
}

// 清空指纹库句柄
const handleClearFingerDb = () => {
  confirmDeleteAction('清空指纹库', '此操作将永久删除所有已录入的指纹数据，且不可恢复！是否继续？', 'finger_db')
}
