...
const handleDelete = (ids) => {
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      ...
      try {
        // ==========================================================
        // [新增] 1. 同步删除 TB_FaceInfo 表中的人脸数据
        // ==========================================================

        // A. 找出选中人员的身份证号 (过滤掉空的)
        const targetIdCards = personList.value
          .filter(p => ids.includes(p.id) && p.id_card)
          .map(p => `'${p.id_card}'`) // 给身份证号加上单引号，变成 '123456'

        // B. 如果有有效的身份证号，执行批量删除
        if (targetIdCards.length > 0) {
          // 拼接 SQL 条件: UserID IN ('ID1', 'ID2', 'ID3')
          const conditionStr = `UserID IN (${targetIdCards.join(',')})`

          console.log('正在清理人脸数据，条件:', conditionStr)
          ...
          await window.electronAPI.el_post({
            ...
            console.warn('人脸数据清理失败 (可能数据不存在或表名错误):', err)
            // 捕获错误，保证即使人脸删不掉，后面的人员档案也能正常删除
          })
        }
        // ==========================================================
