          ...
          <div class="detail-card">
            ...
            <div class="info-table">
              ...
              <div class="info-row">
                ...
                <span class="value">{{ singleItem.self_address }} 号位 / {{ singleItem.lock_self_address === 1 ? '上柜' :
                  '下柜' }}</span>
              </div>
              <div class="info-row">
              ...
              <button class="cyber-btn" :class="{ 'disabled': singleItem.group_status !== '在位' }"
                ...
                <div class="btn-content">
                  ...
                  <div class="text-group">
                    <span class="btn-main-text">{{ singleItem.group_status === '在位' ? '立即领用' : '不可领用' }}</span>
                    <span class="btn-sub-text">{{ singleItem.group_status === '在位' ? '操作留痕 · 错取报警' : '装备已取出 · 禁止操作'
                      }}</span>
                  </div>
                </div>
          ...
          <div class="batch-card">
            ...
            <div class="action-footer">
              <div v-if="selectedItems.length - validItemsCount > 0" class="batch-warning">
                <el-icon>
                  <Warning />
                </el-icon> 系统将自动跳过 {{ selectedItems.length - validItemsCount }} 个不可用装备
              </div>
            ...
            <div class="status-summary" v-if="wrongTakenList.length === 0">
              <span v-if="remainingCount > 0" class="processing-text">
                <el-icon class="is-loading">
                  <Loading />
                </el-icon> 剩余 {{ remainingCount }} 件待取
              </span>
              <span v-else class="success-text">
                <el-icon>
                  <CircleCheck />
                </el-icon> 所有装备已取出，请关闭柜门
              </span>
            </div>
            <div class="status-summary error-text" v-else>
              检测到异常！请放回错误装备
            </div>
...
import {
  Box, SwitchButton, Location, Files, Lock, Unlock,
  CircleCloseFilled, Warning, Check, Mouse, List, Close, Collection, /*Search,*/
  View, Loading, CircleCheck
...
const handleSafeExit = async () => {
  // 1. 如果当前正在领用流程中（弹窗开着），禁止直接退出
  if (borrowProcessVisible.value) {
    ElMessageBox.alert('当前正在进行领用操作，请先完成或通过“人工强制结束”关闭流程后再退出。', '操作受限', {
      type: 'warning',
  ...
  try {
    // 3. 调用全局检测函数
    const isAllClosed = await checkGlobalDoorStatus()
    ...
    if (isAllClosed) {
      ...
      await ElMessageBox.confirm(
        `<div style="color: #ff4d4f; font-weight: bold; font-size: 16px;">检测到柜门未关闭！</div>
         <div style="color: #ccc; margin-top: 10px;">
           离开当前页面将导致<span style="color: #ff4d4f;">系统停止监控</span>，可能造成装备丢失。
         </div>
         <div style="margin-top: 10px; font-size: 12px; color: #8899a6;">
           建议：点击“取消”，去关闭柜门。<br>
           风险：点击“强制退出”，系统将不再负责后续安全监控。
         </div>`,
        '安全警告',
        {
          dangerouslyUseHTMLString: true,
          confirmButtonText: '忽视风险，强制退出',
          cancelButtonText: '取消，我去关门',
    ...
    if (e !== 'cancel') {
      console.error(e)
      ElMessage.error('硬件通讯异常，请检查设备连接')
    }
  }
}
...
const handleStartBorrowProcess = async () => {
  ...
  allInPlaceItems.value = equipmentList.value.filter(item =>
    item.group_status === '在位' && !targetIds.includes(item.id)
  )
...
const getLockRegisterAddress = (item) => {
  ...
  if (config_blob.value) {
    const lockConfig = config_blob.value.lock.details.find(l => l.self_address == item.lock_self_address)
    ...
    try {
      ...
      for (const target of activeBorrowList.value) {
        ...
        if (isRemoved && !target.isTaken) {
          ...
          target.isTaken = false // 如果又放回去了，状态回退
        }
      }
        ...
        await window.electronAPI.el_post({
          ...
          payload: {
            ...
            setValues: {
              ...
              // --- 时间与状态 ---
              borrow_time: timeNow, // 现在使用的是标准格式
              status: 0,    // 0 = 进行中(未归还)
              is_synced: 0, // 0 = 未同步

              // --- 其他 ---
              // [修改] 使用用户选择的 borrowReason
...
const forceExitProcess = () => {
  ...
  // 场景B：还有物品未取出 (异常情况)
    ...
    ElMessageBox.confirm(
      ...
      // [修复点 2] 替换 finalizeBorrow()，并传入 FORCE 标记
...
const secureFinalize = async (entryType = 'NORMAL') => {
  ...
  try {
    ...
    if (isAllClosed) {
      ...
      // B. 门没关 -> 拦截并警告
      audioStore.play('/audio/安全警告检测到柜门未关闭.mp3') // 建议添加此音效
      ...
      if (entryType === 'FORCE') {
        messageHtml = `
          <div style="color: #E6A23C; font-weight: bold;">检测到柜门未关闭！</div>
          <div style="margin-top:8px; color:#ccc;">您正在执行<span style="color:#E6A23C">强制已取</span>操作，但系统检测到柜门仍处于开启状态。</div>
          <div style="margin-top:8px; font-size:12px; color:#aaa;">如果门磁传感器也故障了，请确保物理柜门已关闭后点击强制完成。</div>
        `
        confirmBtnText = '门磁已坏，强制完成'
      } else {
        messageHtml = `
          <div style="color: #ff4d4f; font-weight: bold;">安全警告：柜门未关闭</div>
          <div style="margin-top:8px; color:#ccc;">为了保障资产安全，请务必关闭柜门。</div>
        `
        confirmBtnText = '忽视警告并退出'
      }

      await ElMessageBox.confirm(messageHtml, '安全检查未通过', {
        dangerouslyUseHTMLString: true,
        confirmButtonText: confirmBtnText,
        cancelButtonText: '我去关门',
        confirmButtonClass: 'el-button--danger', // 红色按钮表示高风险
        customClass: 'cyber-message-box error-mode',
        center: true
      })

      // 用户点击了“强制完成/忽视警告”，执行提交
      await finalizeBorrow()
    }
  } catch (e) {
    loading.close()
