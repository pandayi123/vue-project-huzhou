<template>
  <div class="page-container theme-dark">
    ...
    <FaceRegisterModal v-model="faceDialogVisible" :person-name="currentFacePerson.name"
      :id-card="currentFacePerson.idCard" @success="onFaceRegistered" />
...
<script setup>
import { ref, computed, nextTick, watch, reactive, onMounted, defineAsyncComponent, toRaw } from 'vue'
import {
  UserFilled,
  SwitchButton,
  Check,
...
watch(currentInputValue, (newValue, oldValue) => {
  if (showKeyboard.value && activeField.value) {
    ...
    // 针对密码字段进行强制长度限制
    if (activeField.value === 'password' && newValue.length > 8) {
      // 1. 截断新值
      const truncatedValue = newValue.slice(0, 8)

      // 2. 将截断后的值强制写回 currentInputValue
      if (currentInputValue.value !== truncatedValue) {
        currentInputValue.value = truncatedValue

        // 3. 调用子组件 setInput (保持你刚才修复的代码)
        if (keyboardRef.value) {
          keyboardRef.value.setInput(truncatedValue)
        }

        // ================== 【核心修复】开始 ==================
        // 4. 强制修正光标位置
        // 使用 setTimeout(..., 0) 将操作推迟到浏览器渲染周期的最后
        cursorIndex.value = 8

        setTimeout(() => {
          if (activeInputDom.value) {
            // 先 blur 再 focus 有时能解决顽固的光标问题，但在 input 中通常直接 setSelectionRange 配合 setTimeout 即可
            activeInputDom.value.focus()
            // 强行把光标钉在第 8 位
            activeInputDom.value.setSelectionRange(8, 8)
          }
        }, 0) // 0毫秒延时足以让浏览器完成内部的光标回退逻辑
        // ================== 【核心修复】结束 ==================
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      const loading = ElLoading.service({
        lock: true,
        text: '正在加密并保存档案...',
        background: 'rgba(0, 0, 0, 0.8)', // 背景稍微加深一点
      })
...
const handleDelete = (ids) => {
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      ...
      try {
        const response = await window.electronAPI.el_post({
          action: 'remove', // 使用 remove 动作
          payload: {
            tableName: 'users', // 人员表通常是 users
...
// 新增：处理采集成功回调
const onFaceRegistered = (imageData) => {
  console.log('父组件已接收到人脸图像数据:', imageData);
  formData.photo = imageData
  console.log('采集成功')
  // 1. 更新当前表单状态
  formData.has_face = 1

  // 2. 更新列表数据（可选，如果在列表里也有显示人脸状态）
  const target = personList.value.find(p => p.id === formData.id)
  if (target) {
    target.has_face = 1 // 或者是后端对应的字段名
  }
...
const handleRecordFinger = () => {
  if (!isEditMode.value) return
  ElMessage.warning('请将手指放置在指纹采集器上... (需对接硬件SDK)')
  // 这里写对接指纹仪的逻辑
  // 成功后：
  // formData.has_fingerprint = true
}
