    ...
    <div class="network-config-form">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="auto" size="large">
        ...
        <el-form-item label="MAC地址:" prop="macAddress">
          <el-input style="width: 4rem; font-size: 0.28rem;" v-model="mac_address" disabled />
        </el-form-item>
...
// 1. 定义 MAC 地址的响应式变量
const mac_address = ref('ff:ff:ff:ff:ff:ff')
...
const activateKeyboard = (layout, fieldName, event, inputType = 'regular', componentRef = null) => {
  if (activeField.value && inputType == 'regular') {
    ...
    // --- 修复开始 ---
    // 获取当前记录的旧组件ID
    const prevId = activeComponentId.value
    // 只有当 ID 存在，且在 allInputRefs 中能找到对应的组件引用时，才执行操作
    if (prevId && allInputRefs[prevId] && allInputRefs[prevId].value) {
      allInputRefs[prevId].value.setActiveSegment(-1)
    }
  ...
  if (button === '{close}') {
    setTimeout(() => {
      ...
      if (activeField.value) {
        ...
        // --- 修复开始 ---
        // 获取当前记录的组件ID
        const currentId = activeComponentId.value
        // 只有当 ID 存在，且在 allInputRefs 中能找到对应的组件引用时，才执行操作
        if (currentId && allInputRefs[currentId] && allInputRefs[currentId].value) {
          allInputRefs[currentId].value.setActiveSegment(-1)
        }
        // --- 修复结束 ---
      }
    }, 100)
...
*/
// 🔧 新增：初始化网络配置函数
const initializeNetworkConfig = async () => {
  ...
  // --- 新增：获取 MAC 地址 ---
  // 假设你有一个 IPC 方法获取 MAC，或者是直接从 configStore 里拿
  // 如果上一条回答里的 get_current_mac 已经做好了，可以这样调用：
  try {
    const result = await window.electronAPI.el_post({
      action: 'get_current_mac',
      payload: {}
    })
    console.log('获取MAC地址结果:', toRaw(result))
    if (result && result.data) {
      mac_address.value = result.data
      let payload_sql = {
        tableName: 'network_settings',
        setValues: {
          nic_address1: mac_address.value,
        },
        condition: 'id=1',
      }
    ...
    console.error('获取MAC失败', e)
    mac_address.value = 'FF:FF:FF:FF:FF:FF' // 失败时的占位符或保持空
  }
}
const quit_test = () => {
  testItems.value.forEach(item => {
