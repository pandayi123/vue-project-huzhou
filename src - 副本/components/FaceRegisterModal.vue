...
const handleRegister = async () => {
  ...
  try {
    ...
    if (checkResponse?.data?.success) {
      // 场景 A: 库里已经有人脸
      if (props.isUpdate) {
        // -> 如果是更新模式：允许通过，且为了保险，先执行删除旧人脸操作
        statusMessage.value = '检测到旧档案，正在清除旧数据...'

        // 调用删除接口 (假设后端有 face_delete，如果没有，通常直接注册可能会报错“已存在”)
        try {
          let remove_result=await window.electronAPI.el_post({
            action: 'face_remove',
            payload: {
              tableName: 'TB_FaceInfo',
              condition: "UserID = '" + props.idCard + "'"
            }
          })
          console.log('删除旧人脸结果', remove_result)
        } catch (e) {
          console.warn('尝试删除旧人脸失败或无需删除', e)
        }

        shouldRegister = true // 标记为可以继续注册
      } else {
        // -> 如果是新增模式：报错阻止
      ...
      // error_code 2 通常代表“查无此人”，这是我们要的正常状态
      if (checkErrorCode === 2) {
        shouldRegister = true
      } else {
        // 其他错误（如非活体、多人脸等）
    ...
    // --- 第二步：执行注册 ---
    if (shouldRegister) {
      statusMessage.value = props.isUpdate ? '正在更新人脸模型...' : '检测通过，正在录入...'
      ...
      if (registerResponse?.data?.success) {
        statusMessage.value = props.isUpdate ? '更新成功！' : '注册成功！'
