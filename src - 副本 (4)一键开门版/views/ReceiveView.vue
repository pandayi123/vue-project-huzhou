...
<script setup>
import { ref, computed, onMounted, onUnmounted, nextTick, watch, reactive, toRaw } from 'vue'
import {
  Box, SwitchButton, Location, Files, Lock, Unlock,
  CircleCloseFilled, Warning, Check, Mouse, List, Close, Collection, /*Search,*/
  View, Loading, CircleCheck, Tools, Pointer
...
const handleManualOpenDoor = async () => {
  ...
  try {
    ...
    await ElMessageBox.confirm(
      `确认开启柜门并领用 ${selectedIds.value.length > 0 ? '已选' : '全部在位'} 装备吗？`,
      '操作指令确认',
      {
    ...
    // 执行开门逻辑
    // 初始化监控列表
    activeBorrowList.value = targets.map(item => ({
      ...item,
      isTaken: false
    }))

    // 记录误拿比对基准（除了我们要领用的，其他在位的都算误拿）
    const targetIds = targets.map(i => i.id)
    allInPlaceItems.value = equipmentList.value.filter(item =>
      item.group_status === 'en-place' && !targetIds.includes(item.id)
    )
...
const handleSafeExit = async () => {
  ...
  if (borrowProcessVisible.value) {
    ElMessageBox.alert(
      ...
      '操作受限',
      { type: 'warning', customClass: 'cyber-message-box warning-mode', confirmButtonText: '知道了' }
    )
    return
  }
...
const checkItemRemoved = (item) => {
  ...
  if (config_blob.value?.switch?.details) {
    switchConfig = config_blob.value.switch.details.find(
      d => d.self_address == item.self_address
    )
  }
