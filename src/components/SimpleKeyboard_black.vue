...
onMounted(() => {
  keyboard.value = new Keyboard(props.keyboardClass, {
    ...
    onChange: () => {
      // ================== 【核心修复开始】 ==================
      // 1. 获取当前输入
      let currentInput = keyboard.value.getInput()

      // 2. 如果设定了 maxLength 且当前输入超长
      if (props.maxLength > 0 && currentInput.length > props.maxLength) {
        // A. 立即截断字符串
        const truncated = currentInput.slice(0, props.maxLength)

        // B. 立即回写键盘内部状态 (这会移除界面上多出来的那个字符)
        keyboard.value.setInput(truncated)

        // C. 【可选】如果需要提示音，可以在这里播放一个“禁止”音效
        // audioStore.play('/audio/错误.mp3')
      }
      // ================== 【核心修复结束】 ==================

      // 3. 继续执行原有的节流 Emit 逻辑
      // 注意：throttleOnChange 内部会再次调用 keyboard.value.getInput()，
      // 此时获取到的已经是我们刚才 setInput 修正后的合法值了。
